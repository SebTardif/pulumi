...
func newOrgWebhookNewCmdWith(
	...
	cmd := &cobra.Command{
		...
		Example: "  # Create a webhook interactively\n" +
			"  pulumi org webhook new\n\n" +
			"  # Create non-interactively\n" +
			"  pulumi org webhook new --id my-hook --url https://example.com/hook --yes\n\n" +
			"  # Create with a display name and specific event groups\n" +
			"  pulumi org webhook new --id my-hook --name \"My Hook\" \\\n" +
	...
	cmd.Flags().StringVar(&oncmd.id, "id", "",
		"The webhook identifier (1-32 chars, alphanumeric/hyphens/underscores/dots)")
	cmd.Flags().StringVar(&oncmd.name, "name", "",
		"The webhook display name (defaults to the webhook ID)")
...
func (c *orgWebhookNewCmd) run(ctx context.Context) error {
	...
	id := c.id
	if id == "" {
		idErrMsg := "a webhook ID is required (use --id)"
		if !skipPrompts {
			idErrMsg = "a webhook ID is required"
		}
		id, err = ui.PromptForValue(
			skipPrompts, "Webhook ID", "", false,
			func(v string) error {
				if v == "" {
					return errors.New(idErrMsg) //nolint:goerr113 // intentional
				}
				return nil
			}, displayOpts)
		if err != nil {
